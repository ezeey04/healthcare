...
def _remove_readonly(func, path, exc_info):   # to fix windows readonly error
    try:
        os.chmod(path, 0o777)
        func(path)
    except Exception:
...
def clear_local_db_backups():
    if not os.path.exists(BACKUP_DB_DIR):
        return

    shutil.rmtree(BACKUP_DB_DIR, onerror=_remove_readonly)
